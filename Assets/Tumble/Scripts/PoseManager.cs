...
public class PoseManager : UdonSharpBehaviour {
    ...
    public override void PostLateUpdate() {
        ...
        if (_activePose != newPose) {
            ...
            if (_activePose != null
                && (Time.time - _activePose.lastPoseTime > 0.3f || _universe.cheats.NoCooldowns)
                ...
                if (_activePose.groundResetMode == GroundResetMode.Always)
                    ...
                    passed = true;
                else if (_activePose.groundResetMode == GroundResetMode.WhenHasGroundReset && (groundReset || _universe.cheats.InfiniteAirActions))
                    passed                                                                                   = true;
    ...
    private Pose GetActivePose() {
        if (Time.time - _lastJumpTime < 0.2f && !_universe.cheats.NoCooldowns) return null;
        ...
        if (!_universe.cheats.NoCooldowns)
            foreach (var pose in _poses)
