...
namespace Nessie.Udon.Movement
{
    ...
    public class NUMovement : AbstractMovement
    {
        ...
        [PublicAPI] protected bool IsGrounded {
            get => _isGrounded && !forcePlayerUnGrounded;
        ...
        protected virtual void ApplyWalk()
        {
            ...
            if (HoldMove)
            {
                ...
                {
                    _TargetVelocity(movementVector, airAcceleration * (scaleMovement ? AvatarHeight : 1f) * DeltaTime);
                }
            }
            else if (GroundTransform && IsWalkable && !HoldJump && !forcePlayerUnGrounded)
            {
