...
namespace Nessie.Udon.Movement {
    ...
    public class NUMovement : AbstractMovement {
        ...
        [Tooltip("The acceleration when you are in the air.")]
        [SerializeField] protected float airAcceleration = 5f;
        ...
        [PublicAPI] protected bool IsGrounded { get => _isGrounded && !forcePlayerUnGrounded; set => _isGrounded = value; }
        ...
        protected override void ApplyHit(ControllerColliderHit hit) {
            ...
            if (Vector3.Dot(Velocity.normalized, normal) < 0f) {
                if (Vector3.Dot(point - bottomCapCenter, ControllerDown) >= 0f) {
                    ...
                    if (surfaceAngle > slopeLimit) { IsSteep = true; } else {
                        IsWalkable = true;
                ...
                } else if (Vector3.Dot(point - topCapCenter, ControllerUp) >= 0f) { IsJumping = false; }

                if (IsWalkable) { Velocity = HoldMove || HoldJump ? Vector3.ProjectOnPlane(Velocity, -GravityDirection) : Vector3.zero; } else { Velocity = Vector3.ProjectOnPlane(Velocity, normal); }
            }
...
#region VRChat Events
        ...
        public override void OnAvatarEyeHeightChanged(VRCPlayerApi player, float prevEyeHeightAsMeters) {
            ...
            if (AvatarHeight < prevEyeHeightAsMeters) {
                ...
                if (Vector3.Dot(Velocity, -GravityDirection) > 0f) Velocity.y *= ratio;
                Velocity.x *= ratio;
                Velocity.z *= ratio;
            }
        }
...
#region Controls
        ...
        protected virtual void ApplyWalk() {
            ...
            if (HoldMove && !forceNoMovement) {
                ...
                if (IsWalkable && !HoldJump && !forcePlayerUnGrounded) { Velocity = movementVector; } else { _TargetVelocity(movementVector, airAcceleration * (scaleMovement ? AvatarHeight : 1f) * DeltaTime); }
            } else if (GroundTransform && IsWalkable && !HoldJump && !forcePlayerUnGrounded) { Velocity = Vector3.zero; }
        }
        ...
        protected virtual void ApplyJump() {
            if (IsJumping) {
                ...
                if (jumpCancel && !HoldJump) {
                    ...
                    IsJumping = false;
                } else if (JumpTime < jumpApex) { JumpTime += DeltaTime; } else {
                    JumpTime  = jumpApex;
                    IsJumping = false;
                }
            }
        ...
        protected virtual void ApplyGravity() { _AddForce(Gravity * (GravityStrength * (scaleMovement ? AvatarHeight : 1f) * DeltaTime)); }
        ...
        public virtual void ApplyToPlayer() {
            if (MainMenuOpen) {
            ...
            if (playerIsMoving && !IsGrounded || IsSteep || groundIsMoving) {
                ...
                if (InVR && groundIsMoving) vel += GetVROffset() / DeltaTime;
...
#region Input Methods
        ...
        [PublicAPI]
        public virtual void _ControllerDisable() { isActive = false; }
        ...
        [PublicAPI]
        public virtual void _SetCanMove(bool canMove) { InputMoveEnabled = canMove; }

        [PublicAPI]
        public virtual void _SetCanLook(bool canLook) { InputLookEnabled = canLook; }
...
#region Player Methods
        ...
        [PublicAPI]
        public virtual void _Respawn(int spawnsIndex) { LocalPlayer.Respawn(spawnsIndex); }
        ...
        [PublicAPI]
        public virtual void _TeleportTo(Vector3 position, bool lerpOnRemote = false) { _TeleportTo(position, LocalPlayer.GetRotation(), lerpOnRemote); }
        ...
        [PublicAPI]
        public virtual void _SetJumpHeight(float height) { _SetJumpImpulse(Mathf.Sqrt(height * _GetGravityMagnitude() * 2f)); }
        ...
        public virtual void _SetVelocity(Vector3 velocity) {
            Velocity = velocity;
            if (Velocity.y >= _GetGravityMagnitude() * DeltaTime) Unground();
        }
        ...
        [PublicAPI]
        public virtual void _AddForce(Vector3 force) { Velocity += force; }
...
#region Utility Methods
        ...
        public void SnapToPlayer() {
            ...
            } else { playerPos = LocalPlayer.GetPosition(); }
        ...
        [PublicAPI]
        protected Vector3 GetVROffset() { return PlayRotation * LocalPlayPositionDelta * CameraScale; }
